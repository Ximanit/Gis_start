...
function getData(page) {
    ...
    const fields = 'items.point'; // Список полей, которые необходимо получить о каждом объекте
    const url = `https://catalog.api.2gis.com/3.0/items?q=кафе&type=branch&point=${point}&radius=50000&key=rupzho5301&page=${page}&page_size=10&fields=${fields}`;
    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.onload = function () {
            if (xhr.status === 200) { // Если запрос прошел успешно
                ...
                reject(new Error(`Произошла ошибка при запросе данных со страницы ${page}: ${xhr.statusText}`));
            }
        };
        xhr.onerror = function () {
            reject(new Error(`Произошла ошибка при запросе данных со страницы ${page}`));
        };
...
function saveData(data) {
    const fileName = 'cafes.json'; // Имя файла для записи данных

    try {
        const existingData = JSON.parse(localStorage.getItem(fileName) || '{"result": {"items": []}}'); // Получаем данные из localStorage и преобразуем в объект
        ...
        existingData.result.items = existingData.result.items.concat(data.result.items);

        localStorage.setItem(fileName, JSON.stringify(existingData)); // Записываем данные в localStorage
    } catch (error) {
        console.log('Произошла ошибка при записи данных в localStorage:', error.message);
    }
}
...
async function processPages() {
    ...
    while (true) {
        try {
            ...
            if (data === undefined || !data.result || !data.result.items || !data.result.items.length) break; // Если данные закончились, выходим из цикла

            saveData(data); // Сохраняем данные в localStorage

            console.log(`Страница ${page}: получено ${data.result.items.length} объектов`);
...
(function () {
    const fileName = 'cafes.json'; // Имя файла, в который будут записаны данные
    const data = JSON.parse(localStorage.getItem(fileName)); // Получаем данные из localStorage

    if (!data || !data.result.items.length) { // Проверяем наличие данных и объектов в массиве
